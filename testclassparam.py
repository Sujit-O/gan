...
class hparam:
    ...
    def printhparam(self):
        ...
        print ('                 Z sample size : ', self.Z_dim)
    ...
    def savehparam(self):
        ...
        outhparampath= self.outdir+'/hparams.txt'
        if not os.path.exists(outhparampath):
            number=0
            for file in os.listdir(self.outdir):
             if 'hparams' in file:
                 number+=1
            outhparampath=   self.outdir+'/hparams'+str(number)+'.txt'
    ...
    def loadhparam(self):
        ...
        if not os.path.exists(outhparampath):
            ...
            for file in os.listdir(self.outdir):
             if 'hparams' in file:
                 number+=1
            outhparampath=   self.outdir+'/hparams'+str(number-1)+'.txt'
